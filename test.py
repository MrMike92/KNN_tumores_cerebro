...
class MostrarImagen:
    ...
    def mostrarEscalaGrises(self, nuevo_ancho, nuevo_alto):
        ...
        imagen_gris_redimensionada = imagen_gris.resize((nuevo_ancho, nuevo_alto))  # Redimensionar a 64x64 la imagen en escala de grises

        ''' imagen_gris_redimensionada.show() # Muestra de imagen redimensionada de escala de grises'''

        matriz = list(imagen_gris_redimensionada.getdata())  # Obtiener la matriz de la imagen a escala de grises

        '''print("Matriz de la imagen en escala de grises redimensionada:")  # Imprimir la matriz de la imagen a escala de grises'''
        ...
        # Obtener la etiqueta según el nombre de la imagen
        nombre_imagen = os.path.basename(self.ruta_imagen)
        if nombre_imagen.startswith("b"):
            etiqueta = 1
        elif nombre_imagen.startswith("p"):
            etiqueta = 0
        else:
            etiqueta = -1  # Valor por defecto para casos no contemplados
...
def main():
    ...
    # Procesar las imágenes de la carpeta "test"
    nombres_archivos_pajaros = os.listdir(ruta_Test)
    for nombre_imagen in nombres_archivos_pajaros:
        ruta_imagen = os.path.join(ruta_Test, nombre_imagen)
        mostrar_Test = MostrarImagen(ruta_imagen)
        resultados = mostrar_Test.mostrarEscalaGrises(64, 64)
